...
int verification (Int_t testID = 3, TString testDataFilePath = "m115-test3-1.txt") {
	...
	const Bool_t debugCountLines		= true;			// this wil print every counter value
	...
	// This section of the code will load the .txt file and start loading the data
	...
	while (true) {
		...
		// technical stuff after/before each loop
		counter++;
		if (debugCountLines && !debugTotalSilence) cout << counter << endl;
		if (counter>=timeout) break;
		else if (counter <= 4 || counter == 6) continue;        //this section IGNORES all settings, since they are not really necessary
		...
		if (counter == 5) { //load nPads and nReps, initialize and define all histograms
